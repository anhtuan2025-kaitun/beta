...
-- FIX CHO DELTA / HYDROGEN MOBILE  
local req = (http_request or request or syn and syn.request)
if not req then
    return warn("⚠️ Executor không hỗ trợ HTTP Request!")
end
...
local function getScript(url)
    ...
    if res.Body:find("<html") then
        return nil
    end
    return res.Body
end

local scriptData = getScript("https://raw.githubusercontent.com/viplam087-png/Hjj/main/AttackAndBring.lua")

if not scriptData then
    return warn("❌ Không tải được file AttackAndBring.lua — GitHub từ chối mobile hoặc mạng yếu!")
end
